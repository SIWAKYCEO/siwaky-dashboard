...
/**
 * Proxies FastAPI `POST /push/subscribe` for authenticated dashboard sessions only.
 */
export async function POST(req: Request) {
  ...
  const upstream = `${upstreamBase}/push/subscribe`;
  try {
    ...
    if (!res.ok) {
      return NextResponse.json(
        { error: "Push subscribe failed", detail: text.slice(0, 400), upstream },
    ...
    return NextResponse.json({ error: "Push subscribe upstream unreachable", detail: message, upstream }, { status: 503 });
  }
}
